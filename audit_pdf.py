...
def _proposal_metrics(audit: dict):
    ...
    acc_lh   = lh.get("accessibility",  98)
    bp_lh    = lh.get("best_practices", 96)
    ...
    return dict(
        ...
        lcp_b=lcp_b,   lcp_a=lcp_a,
        fcp_b=fcp_b,   fcp_a=fcp_a,
...
class ProposalPage(Flowable):
    ...
    def _render(self, c):
        ...
        # ── Header ────────────────────────────────────────────────────────────
        prepared = f"PREPARED FOR {m['biz'].upper()} · CONFIDENTIAL" if m['biz'] else "CONFIDENTIAL"
        _draw_page_header(c, W, H, LOGO_PATH,
                          "WEBSITE PERFORMANCE PROPOSAL", prepared)
        ...
        pill_y = H - 90
        c.setFillColor(m['opp_col'])
        c.roundRect(cx - pill_w / 2, pill_y, pill_w, pill_h, 8, fill=1, stroke=0)
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 8.5)
        c.drawCentredString(cx, pill_y + 25, "FAST.SITE OPPORTUNITY SCORE")
        c.setFont("Helvetica-Bold", 14)
        c.drawCentredString(cx, pill_y + 9, f"{m['opp_score']}/100 · {m['opp_label']}")

        # ── "MEASURED SPEED GAIN" green badge above circle ────────────────────
        badge_w, badge_h = 190, 22
        badge_y = pill_y - 30
        c.setFillColor(GREEN)
        c.roundRect(cx - badge_w / 2, badge_y, badge_w, badge_h, 6, fill=1, stroke=0)
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 9)
        c.drawCentredString(cx, badge_y + 7, "MEASURED SPEED GAIN")

        # ── Speed gain circle ─────────────────────────────────────────────────
        circle_r  = 78
        circle_cy = badge_y - circle_r - 6
        # outer dark ring
        c.setFillColor(colors.HexColor("#1B3A2E"))
        c.circle(cx, circle_cy, circle_r, fill=1, stroke=0)
        # inner darker ring
        c.setFillColor(colors.HexColor("#0D2A1E"))
        c.circle(cx, circle_cy, circle_r - 20, fill=1, stroke=0)
        # big green percentage
        c.setFillColor(GREEN)
        c.setFont("Helvetica-Bold", 46)
        c.drawCentredString(cx, circle_cy + 4, f"{m['ttfb_pct']}%")
        # subtitle lines
        c.setFillColor(colors.HexColor("#94A3B8"))
        c.setFont("Helvetica", 8.5)
        c.drawCentredString(cx, circle_cy - 16, "faster time to first byte")
        c.setFillColor(colors.HexColor("#64748B"))
        c.setFont("Helvetica", 7.5)
        c.drawCentredString(cx, circle_cy - 28, f"{m['ttfb_b']}ms -> {m['ttfb_a']}ms")

        # ── Three metric cards ────────────────────────────────────────────────
        card_top = circle_cy - circle_r - 14
        card_h   = 58
        card_gap = 8
        card_w   = (W - 2 * L - card_gap * 2) / 3
        cards = [
            ("TIME TO FIRST BYTE", f"{m['ttfb_a']}ms",
             f"from {m['ttfb_b']}ms · {m['ttfb_pct']}% faster", BLUE),
            ("FULL PAGE LOAD",     f"{m['load_a']}ms",
             f"from {m['load_b']}ms · {m['load_pct']}% faster", BLUE),
            ("GLOBAL EDGE",        f"{m['edge']} regions",
             "SSL · HTTP/3 · DDoS + WAF",
             colors.HexColor("#06B6D4")),
        ]
        for i, (title, value, subtitle, accent) in enumerate(cards):
            bx = L + i * (card_w + card_gap)
            by = card_top - card_h
            c.setFillColor(colors.HexColor("#12111E"))
            c.roundRect(bx, by, card_w, card_h, 4, fill=1, stroke=0)
            # left accent bar
            c.setFillColor(accent)
            c.roundRect(bx, by, 3, card_h, 2, fill=1, stroke=0)
            # label
            c.setFillColor(colors.HexColor("#64748B"))
            c.setFont("Helvetica-Bold", 6)
            c.drawString(bx + 10, by + card_h - 14, title)
            # value — green for TTFB/load, cyan for edge
            val_col = accent if i == 2 else GREEN
            c.setFillColor(val_col)
            c.setFont("Helvetica-Bold", 20)
            c.drawString(bx + 10, by + card_h - 36, value)
            # subtitle
            c.setFillColor(colors.HexColor("#475569"))
            c.setFont("Helvetica", 6.5)
            c.drawString(bx + 10, by + 9, subtitle)

        # ── Domain / URL / date ───────────────────────────────────────────────
        sep1_y = card_top - card_h - 16
        c.setStrokeColor(colors.HexColor("#2D2B3D"))
        c.setLineWidth(0.75)
        c.line(L, sep1_y, W - R, sep1_y)
        ...
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 20)
        c.drawString(L, sep1_y - 24, m['disp'][:50])
        # blue URL below
        c.setFillColor(BLUE_DIM)
        c.setFont("Helvetica", 9)
        c.drawString(L, sep1_y - 37, m['url'][:70])
        # report date right-aligned on same line as domain
        c.setFillColor(colors.HexColor("#64748B"))
        c.setFont("Helvetica", 8)
        c.drawRightString(W - R, sep1_y - 24, f"Report date: {m['date_str']}")

        # ── KEY FINDINGS ──────────────────────────────────────────────────────
        kf_label_y = sep1_y - 55
        c.setFillColor(colors.HexColor("#93B5FF"))
        c.setFont("Helvetica-Bold", 8)
        c.drawString(L, kf_label_y, "KEY FINDINGS")
        # measured signals count right-aligned
        n_sig = sum(1 for v in [m['ttfb_b'], m['load_b']] if v)
        c.setFillColor(colors.HexColor("#64748B"))
        c.setFont("Helvetica", 7.5)
        c.drawRightString(W - R, kf_label_y, f"{n_sig} measured signals")

        findings = []
        if m['ttfb_b'] and m['ttfb_b'] > 200:
            findings.append(("HIGH", AMBER,
                              f"Server response above target: {m['ttfb_b']}ms TTFB (target <200ms)"))
        if m['ttfb_pct'] > 0:
            findings.append(("GOOD", GREEN,
                              f"Fast.site preview is {m['ttfb_pct']}% faster to first byte"))
        # extra signals from lighthouse
        if m['perf_lh'] and m['perf_lh'] < 50:
            findings.append(("HIGH", AMBER,
                              f"Performance score critical: {m['perf_lh']}/100"))
        if m['seo_lh'] and m['seo_lh'] < 70:
            findings.append(("MEDIUM", SKY,
                              f"SEO score needs attention: {m['seo_lh']}/100"))

        row_h = 19; frow_y = kf_label_y - 16
        badge_fw = 44
        for sev, col, text in findings[:4]:
            # row background
            c.setFillColor(colors.HexColor("#13121F"))
            c.roundRect(L, frow_y - 4, W - L - R, row_h, 3, fill=1, stroke=0)
            # coloured badge
            c.setFillColor(col)
            c.roundRect(L + 5, frow_y + 1, badge_fw, 12, 3, fill=1, stroke=0)
            c.setFillColor(WHITE)
            c.setFont("Helvetica-Bold", 6)
            c.drawCentredString(L + 5 + badge_fw / 2, frow_y + 5, sev)
            # finding text
            c.setFillColor(colors.HexColor("#CBD5E1"))
            c.setFont("Helvetica", 7.5)
            c.drawString(L + badge_fw + 14, frow_y + 4, text[:90])
            frow_y -= row_h + 2
        ...
        c.setStrokeColor(colors.HexColor("#2D2B3D"))
        c.setLineWidth(0.4)
        c.line(L, lh_sep_y, W - R, lh_sep_y)

        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 8.5)
        c.drawString(L, lh_sep_y - 14, "LIGHTHOUSE LAB SCORES")
        c.setFillColor(colors.HexColor("#64748B"))
        c.setFont("Helvetica", 6.5)
        c.drawRightString(W - R, lh_sep_y - 14,
                          "supporting context · single-pass lab, varies run-to-run")

        lh_items = [
            ("PERFORMANCE",    m['perf_lh']),
            ("SEO",            m['seo_lh']),
            ("ACCESSIBILITY",  m['acc_lh']),
            ("BEST PRACTICES", m['bp_lh']),
        ]
        lh_gap   = 6
        lh_cw    = (W - 2 * L - lh_gap * 3) / 4
        lh_ch    = 62
        lh_by    = lh_sep_y - 22 - lh_ch

        for i, (lbl, sc) in enumerate(lh_items):
            lx = L + i * (lh_cw + lh_gap)
            col = _sc(sc)
            c.setFillColor(colors.HexColor("#12111E"))
            c.roundRect(lx, lh_by, lh_cw, lh_ch, 4, fill=1, stroke=0)
            # bottom green bar (score bar)
            bar_w = lh_cw - 14
            fill_w = max(int(sc / 100 * bar_w), 3)
            c.setFillColor(colors.HexColor("#1E1C2D"))
            c.roundRect(lx + 7, lh_by + 8, bar_w, 4, 2, fill=1, stroke=0)
            c.setFillColor(col)
            c.roundRect(lx + 7, lh_by + 8, fill_w, 4, 2, fill=1, stroke=0)
            # big score number
            c.setFillColor(col)
            c.setFont("Helvetica-Bold", 28)
            sc_str = str(sc)
            c.drawString(lx + 9, lh_by + 20, sc_str)
            # "/100" in grey
            num_w = c.stringWidth(sc_str, "Helvetica-Bold", 28)
            c.setFillColor(colors.HexColor("#94A3B8"))
            c.setFont("Helvetica", 8)
            c.drawString(lx + 9 + num_w + 2, lh_by + 28, "/100")
            # label top-left
            c.setFillColor(colors.HexColor("#64748B"))
            c.setFont("Helvetica-Bold", 5.5)
            c.drawString(lx + 9, lh_by + lh_ch - 13, lbl)
...
class ProjectionPage(Flowable):
    ...
    def _render(self, c):
        ...
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 14)
        c.drawString(L, y, "Fast.site performance projection")
        c.setFillColor(colors.HexColor("#94A3B8"))
        c.setFont("Helvetica", 8)
        c.drawString(L, y - 14,
            "Measured from your site's real metrics. "
            "Server delivery is what the edge controls. No code changes required.")

        # ── Three big stat boxes ──────────────────────────────────────────────
        box_top = y - 30
        box_h   = 56
        box_gap = 8
        box_w   = (W - 2 * L - box_gap * 2) / 3
        stats = [
            (f"{m['ttfb_pct']}%", "TTFB REDUCTION",       "Server response time"),
            (f"{m['load_pct']}%", "FULL LOAD REDUCTION",  "Time to last byte"),
            (str(m['edge']),      "GLOBAL EDGE REGIONS",  "SSL · HTTP/3 · DDoS + WAF"),
        ]
        for i, (val, lbl, sub) in enumerate(stats):
            bx = L + i * (box_w + box_gap)
            by = box_top - box_h
            c.setFillColor(colors.HexColor("#1A1830"))
            c.roundRect(bx, by, box_w, box_h, 4, fill=1, stroke=0)
            c.setStrokeColor(colors.HexColor("#2D2B3D"))
            c.setLineWidth(0.5)
            c.roundRect(bx, by, box_w, box_h, 4, fill=0, stroke=1)
            # big value
            c.setFillColor(BLUE_DIM)
            c.setFont("Helvetica-Bold", 26)
            c.drawCentredString(bx + box_w / 2, by + box_h - 28, val)
            # label
            c.setFillColor(WHITE)
            c.setFont("Helvetica-Bold", 7)
            c.drawCentredString(bx + box_w / 2, by + box_h - 40, lbl)
            # sub
            c.setFillColor(GRAY)
            c.setFont("Helvetica", 6.5)
            c.drawCentredString(bx + box_w / 2, by + 10, sub)

        # ── Metrics comparison table ──────────────────────────────────────────
        tbl_top = box_top - box_h - 10
        tbl_row_h = 15
        col_xs = [L, L + (W - 2*L)*0.38, L + (W - 2*L)*0.58, W - R - 70]
        hdrs = ["METRIC", "TODAY", "FAST.SITE", "IMPROVEMENT"]
        hdr_y = tbl_top

        # header background
        c.setFillColor(colors.HexColor("#1A1830"))
        c.roundRect(L, hdr_y - 4, W - 2*L, tbl_row_h + 2, 3, fill=1, stroke=0)
        c.setStrokeColor(colors.HexColor("#2D2B3D"))
        c.setLineWidth(0.4)
        c.roundRect(L, hdr_y - 4, W - 2*L, tbl_row_h + 2, 3, fill=0, stroke=1)
        for j, hdr in enumerate(hdrs):
            c.setFillColor(BLUE_DIM)
            c.setFont("Helvetica-Bold", 7)
            c.drawString(col_xs[j] + 4, hdr_y + 2, hdr)

        ps_gain_str = f"+{m['ps_gain']} pts" if m['ps_gain'] else f"+{m['ps_a'] - m['ps_b']} pts"
        rows = [
            ("Server response (TTFB)", f"{m['ttfb_b']}ms",   f"{m['ttfb_a']}ms",   f"{m['ttfb_pct']}% faster", GREEN),
            ("Full load (TTLB)",       f"{m['load_b']}ms",   f"{m['load_a']}ms",   f"{m['load_pct']}% faster", GREEN),
            ("Payload size",           f"{m['payload']} KB", f"{m['payload']} KB", "0% lighter",               colors.HexColor("#06B6D4")),
            ("Edge cache hit rate",    "—",                  f"{m['cache']}%",     "—",                         GRAY),
            ("PageSpeed score (lab)",  f"{m['ps_b']}/100",   f"{m['ps_a']}/100",   ps_gain_str,                 GREEN),
        ]
        for k, (metric, before, after, uplift, ucol) in enumerate(rows):
            row_y = hdr_y - (k + 1) * tbl_row_h - 4
            bg = colors.HexColor("#13121F") if k % 2 == 0 else colors.HexColor("#0F0E1A")
            c.setFillColor(bg)
            c.rect(L, row_y - 2, W - 2*L, tbl_row_h, fill=1, stroke=0)
            c.setFillColor(colors.HexColor("#94A3B8"))
            c.setFont("Helvetica", 7.5)
            c.drawString(col_xs[0] + 4, row_y + 3, metric)
            c.setFillColor(colors.HexColor("#64748B"))
            c.drawString(col_xs[1] + 4, row_y + 3, before)
            c.setFillColor(WHITE)
            c.setFont("Helvetica-Bold", 7.5)
            c.drawString(col_xs[2] + 4, row_y + 3, after)
            c.setFillColor(ucol)
            c.drawString(col_xs[3] + 4, row_y + 3, uplift)

        # ── Before vs after bar charts ────────────────────────────────────────
        bva_top = hdr_y - len(rows) * tbl_row_h - 22
        c.setFillColor(WHITE)
        c.setFont("Helvetica-Bold", 9)
        c.drawString(L, bva_top, "Before vs after")
        ...
        charts = [
            ("TIME TO FIRST BYTE", m['ttfb_b'], m['ttfb_a'],
             f"{m['ttfb_b']}ms", f"{m['ttfb_a']}ms"),
            ("FULL PAGE LOAD",     m['load_b'], m['load_a'],
             f"{m['load_b']}ms", f"{m['load_a']}ms"),
        ]
        cy_bar = bva_top - 18
        for sec_title, before_v, after_v, before_s, after_s in charts:
            # section title
            c.setFillColor(colors.HexColor("#64748B"))
            c.setFont("Helvetica-Bold", 6)
            ...
            for row_label, val, val_str, is_after in [
                ("Your origin",    before_v, before_s, False),
                ("Fast.site edge", after_v,  after_s,  True),
            ]:
                c.setFillColor(colors.HexColor("#94A3B8"))
                c.setFont("Helvetica", 6.5)
        ...
        c.setFillColor(colors.HexColor("#64748B"))
        c.setFont("Helvetica", 7)
        c.drawString(L + 110, cwv_y + 1,
            "— lab, supporting    single-pass Lighthouse · ranking context, not the headline")

        cwv_card_gap = 10
        cwv_cw  = (W - 2*L - cwv_card_gap * 2) / 3
        cwv_ch  = 100
        cwv_by  = cwv_y - 16 - cwv_ch

        def _fmt_ms(v):
            return f"{v/1000:.1f}s" if v >= 1000 else f"{v}ms"

        def _cwv_pass(val, good):
            return val <= good

        cwv_items = [
            ("LCP", "Largest Contentful Paint", m['lcp_b'], m['lcp_a'], 2500,
             _fmt_ms(m['lcp_b']), _fmt_ms(m['lcp_a']), f"Google good: <2.5s"),
            ("FCP", "First Contentful Paint",   m['fcp_b'], m['fcp_a'], 1800,
             _fmt_ms(m['fcp_b']), _fmt_ms(m['fcp_a']), f"Google good: <1.8s"),
            ("TTFB","Time to First Byte",        m['ttfb_b'], m['ttfb_a'], 800,
             _fmt_ms(m['ttfb_b']), _fmt_ms(m['ttfb_a']), f"Google good: <800ms"),
        ]

        for i, (abbr, full, cur_v, aft_v, good_t, cur_s, aft_s, good_s) in enumerate(cwv_items):
            cx2 = L + i * (cwv_cw + cwv_card_gap)
            passing = _cwv_pass(cur_v, good_t)
            status_col = GREEN if passing else AMBER
            status_txt = "PASS" if passing else "NEEDS WORK"
            ...
            # PASS / NEEDS WORK badge top-right — width follows text so it never overflows
            badge_font_size = 6.5
            badge_pad_x = 5
            ...
            c.setFillColor(colors.HexColor("#94A3B8"))
            c.setFont("Helvetica-Bold", 9)
            c.drawString(cx2 + 9, cwv_by + cwv_ch - 20, abbr)
            # full name
            c.setFillColor(colors.HexColor("#64748B"))
            c.setFont("Helvetica", 6.5)
            c.drawString(cx2 + 9, cwv_by + cwv_ch - 31, full)

            # big current value
            c.setFillColor(status_col)
            c.setFont("Helvetica-Bold", 24)
            c.drawString(cx2 + 9, cwv_by + 52, cur_s)

            # google good target
            c.setFillColor(colors.HexColor("#475569"))
            c.setFont("Helvetica", 6.5)
            c.drawString(cx2 + 9, cwv_by + 40, good_s)

            # divider
            c.setStrokeColor(colors.HexColor("#2D2B3D"))
            c.setLineWidth(0.4)
            c.line(cx2 + 9, cwv_by + 34, cx2 + cwv_cw - 9, cwv_by + 34)

            # "with fast.site" projected value
            c.setFillColor(GREEN)
            c.setFont("Helvetica-Bold", 9)
            c.drawString(cx2 + 9, cwv_by + 20, aft_s)
            c.setFillColor(colors.HexColor("#64748B"))
            c.setFont("Helvetica", 7)
            c.drawString(cx2 + 9 + c.stringWidth(aft_s, "Helvetica-Bold", 9) + 4,
                         cwv_by + 20, "with fast.site")
...
def _story(audit: dict) -> list:
    story = []
    # Page 1: opportunity score / speed-gain proposal sheet
    story.append(ProposalPage(audit))
    story.append(PageBreak())
    # Page 2: performance projection (origin vs fast.site edge)
    story.append(ProjectionPage(audit))
    return story


# ─── PUBLIC API ───────────────────────────────────────────────────────────────
def generate_audit_pdf(audit: dict, lang: str = "en") -> bytes:
    global _LANG
